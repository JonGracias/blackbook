...
def search():
    ...
    for keys in contactsdictionary1:
        if name in keys:
            print((DASH + "|{:<20s} | {:>8s} | {:^12s} | {:^20s} {:>6s}".format(
                "NAME", "RELATIONSHIP", "PHONE", "EMAIL", "|") + DASH))

            print((DASH + "|{:<20s} | {:^12s} | {:^12s} | {:<25s} {:<0s}" + DASH.format(keys.upper(), contactsdictionary1[keys]["relationship"].upper(),
                contactsdictionary1[keys]["tel"], contactsdictionary1[keys]["email"].upper(), "|") + DASH
                found += 1
                if found == 0:
                ...
                if contactdictionary2 != contactsdictionary1:
                save=input(
                DASH + "WOULD YOU LIKE TO SAVE CHANGES BEFORE QUITTING? | Y | N |" + DASH).lower()
                if save == "y":
                save()
                running=False
                sys.exit()
